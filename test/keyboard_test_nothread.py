...
def getObjects(frame, thres, nms, draw=True, objects=[]):
    classIds, confs, bbox= net.detect(frame, confThreshold=thres, nmsThreashold =nms)
    robotMode = 1
    if len(objects) == 0:
        objects = classNames
    objectInfo = []
...
def disinfect_keyboard(_roomba):
    ...
    time.sleep(1)
    roomba.start()
    print("Starting high touch surface cleaning...")
    time.sleep(1)
    ...
    try:
        while True:
            ...
            # if search_other_keyboard:
            #     # cv2.destroyAllWindows()
            #     cv2.imshow('Output', frame)
            #     cv2.waitKey(1)
            #     print("Searching other keyboard")
            #     rotate180(1)
            #     search_other_keyboard = False
